...
import {
  connectExistingResellerAsaasAction,
  disconnectResellerAsaasAction,
} from "./actions"

export default async function ResellerPaymentsSettingsPage({
  searchParams,
}: {
  ...
  const { data: profile } = await supabase
    .from("profiles")
    .select("asaas_wallet_id,asaas_api_key_hint,asaas_is_subaccount,asaas_account_id")
    .eq("id", user.id)
    .single()
  ...
  return (
    <div className="flex-1 space-y-6 p-6 md:p-8 pt-6 max-w-5xl mx-auto w-full">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Conecte sua conta para receber o lucro automaticamente via split.</p>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Dados atuais de integração.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-2">
          ...
          {isConnected && (
            <form action={disconnectResellerAsaasAction} className="pt-2">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Se você ainda não tem conta no Asaas, crie uma e depois cole sua API key aqui.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="text-sm text-muted-foreground">
            <a href="https://www.asaas.com/" target="_blank" rel="noopener noreferrer" className="underline underline-offset-4">
              Criar conta no Asaas
            </a>
          </div>
          <form action={connectExistingResellerAsaasAction} className="space-y-4">
