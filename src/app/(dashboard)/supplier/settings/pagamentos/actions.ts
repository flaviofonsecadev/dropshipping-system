...
export async function connectSupplierAsaasAction(formData: FormData) {
  ...
  if (!apiKey) {
    redirect("/supplier/settings/pagamentos?error=missing_api_key")
  }
  ...
  try {
    ...
    if (error) {
      redirect("/supplier/settings/pagamentos?error=save_failed")
    }
  } catch (e) {
    if (e instanceof AsaasError) {
      if (e.code === "wallet_parse_error" || e.code === "wallet_unknown_error") {
        redirect("/supplier/settings/pagamentos?error=wallet_unavailable")
      }
      redirect(`/supplier/settings/pagamentos?error=asaas_${e.status}`)
    }
    redirect("/supplier/settings/pagamentos?error=invalid_api_key")
  }

  redirect("/supplier/settings/pagamentos?saved=1")
}

export async function disconnectSupplierAsaasAction() {
  const supabase = await createClient()
  const {
    data: { user },
  } = await supabase.auth.getUser()
